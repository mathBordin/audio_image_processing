...
for k1 = 1:N(1)
   for k2 = 1:N(2)
       H1(k1,k2) = 1-exp(-((k1-m1)^2+(k2-m2)^2)/(2*sigma2_1));
       H2(k1,k2) = 1-exp(-((k1-m1)^2+(k2-m2)^2)/(2*sigma2_2));
       H3(k1,k2) = 1-exp(-((k1-m1)^2+(k2-m2)^2)/(2*sigma2_3));
   end
end
% Plota resposta em frequência dos filtros
figure(3);
imagesc(H1);
title('Espectro centralizado do filtro H1 (sigma^2 = 300)');
figure(4);
imagesc(H2);
title('Espectro centralizado do filtro H2 (sigma^2 = 1000)');
figure(5);
imagesc(H3);
title('Espectro centralizado do filtro H3 (sigma^2 = 2000)');
...
title('Espectro centralizado da imagem filtrada (sigma^2 = 300)');
...
title('Imagem filtrada (sigma^2 = 300)');
...
figure(8);
imagesc(log(abs(fft_filt)+1));
title('Espectro centralizado da imagem filtrada (sigma^2 = 1000)');
...
figure(9);
imshow(img_filt,'InitialMagnification', 'fit');
title('Imagem filtrada (sigma^2 = 1000)');
...
figure(10);
imagesc(log(abs(fft_filt)+1));
title('Espectro centralizado da imagem filtrada (sigma^2 = 2000)');
...
figure(11);
imshow(img_filt,'InitialMagnification', 'fit');
title('Imagem filtrada (sigma^2 = 2000)');
